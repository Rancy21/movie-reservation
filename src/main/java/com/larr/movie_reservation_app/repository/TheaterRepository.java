...
public interface TheaterRepository extends JpaRepository<Theater, String> {
    @Query("select new com.larr.movie_reservation_app.dto.TheaterDetailDTO(t.id, t.name, t.address, t.city, t.totalScreens) "
            +
            "from Theater t where t.id=:id")

    Optional<TheaterDetailDTO> findByIdAsDto(@Param("id") String id);

    @Query("select new com.larr.movie_reservation_app.dto.TheaterDTO(t.id, t.name, t.city)"
            +
            "from Theater t where t.isActive = true")
    Page<TheaterDTO> listAllActive(Pageable pageable);

    @Query("select new com.larr.movie_reservation_app.dto.TheaterDTO(t.id, t.name, t.city) from Theater t")
